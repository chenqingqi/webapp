 ...
 * 主控制器
 */
module('required_control',function()
{
	var model = reg('required_model');
	var mid;
	var oauth_token;
	...
	function complete()
	{
		...
		{	
			...
			request.connect({
				...
				page:pages,
				pageNum:10
			},config.api.requiredcourse,'get',config.service,setresourcelist)
	   ...
	   function loginTime(){
   	    ...
   	    uitishi.text('登陆超时，请重新登录');
		     ...
		     window.setTimeout(function()
				{
					uitishi.hide();
					window.location.href='/app/login/login.html?from=requiredcourse';
				},1000);
       }
		return{
