...
module('search_homeview',function()
{
	...
	{   	
	    ...
	    $('#bar_title_search').attr('hidden',false);
        var H=($(window).height()-213)/2;
	    $('#panel_empty').css('margin-top',H+'px');
	...
	function searchcurserlist(){
		...
		if(datesearch.length>0){
			...
			for(var i=0;i<datesearch.length;i++){
	            ...
	            var searchui='<a href="/app/course/course.html?treeid='+datesearch[i].courseId+'&publicCourse='+datesearch[i].publicCourse+'"><div id="item_course_full"><div id="logo"><img src="'+pic+'"></div><div id="container"><p id="title">'+datesearch[i].title+'</p><span id="price" class="'+priceClass+'">'+price+'</span><span id="hour">'+datesearch[i].timeLength+'课时</span></div></div></a>'
	...
	function addloding() {
	    if(myScroll.directionY == 1) {
			if(myScroll.y==myScroll.maxScrollY) {
				...
				if(lodingnumber2==2){}
	              	lodingnumber1=0;
	              	if(pages<model.searchcurser().totalPage){
