 ...
 * 主控制器
 */
module('mycourselist_control',function()
{
	...
	var request = reg('request');
		...
		{	
			...
			request.connect({
				...
				page:pages,
				pageNum:10
			},config.api.mycourselist,'post',config.service,setresourcelist)
	   ...
	   function loginTime(){
   	    ...
   	    uitishi.text('登陆超时，请重新登录');
		     ...
		     window.setTimeout(function()
				{
					uitishi.hide();
					window.location.href='/app/login/login.html?from=mycourselist';
				},1000);
       }
		return{
